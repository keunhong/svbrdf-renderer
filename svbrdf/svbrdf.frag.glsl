...
void main() {
    ...
    vec3 specv = texture2D(spec_shape_map, v_uv).rgb * 100;
        //gl_FragColor = vec4(vec3(specv.x), 1.0);	// rough gamma
//return;
    ...
    vec3 N = normalize(texture2D(normal_map, v_uv).rgb);
    ...
    float spec = exp(-pow(dot(HnpW.xy,Hnp.xy), 1.926*0.5));
    ...
    vec3 radiance = (vec3(spec) * alb_s + alb_d)*vec3(cosine)/D2 * 1500.0 * vec3(1.0);
